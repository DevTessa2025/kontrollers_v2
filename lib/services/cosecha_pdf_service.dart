...
class CosechaPDFService {
  ...
  // Items específicos para cosecha (20 items del 1 al 20)
  static final List<int> ITEMS_COSECHA = List.generate(20, (index) => index + 1);

  static Future<Uint8List> generate({
    required Map<String, dynamic> data,
    bool obtenerDatosFrescos = false,
  }) async {
    print('[PDF][COSECHA] Generando PDF para cosecha ID=${data['id']} finca=${data['finca_nombre']}');
    ...
    for (final ruta in rutasPosibles) {
      try {
        ...
        print('[PDF][COSECHA] Banner cargado desde: $ruta');
        break;
      } catch (_) {}
    }

    // Obtener items relevantes para cosecha
    final List<Map<String, dynamic>> itemsRelevantes = _extraerItemsRelevantesCosecha(data);
    ...
    print('[PDF][COSECHA] PDF generado exitosamente');
    return pdf.save();
  }
    ...
    return pw.Container(
      ...
      child: pw.Row(
        ...
        children: [
          ...
          pw.Text('COSECHA', style: pw.TextStyle(color: PdfColors.white, fontWeight: pw.FontWeight.bold, fontSize: 14)),
        ],
      ),
    );
  }
  ...
  static pw.Widget _construirResumenCumplimiento(Map<String, dynamic> data) {
    ...
    print('[PDF][COSECHA] Calculando porcentaje de cumplimiento...');
    
    for (int i in ITEMS_COSECHA) {
      // Usar el mismo formato que el servicio general
      final respuesta = data['item_${i}_respuesta']?.toString();
      print('[PDF][COSECHA] Item $i: respuesta=$respuesta');
    ...
    print('[PDF][COSECHA] Total items: $totalItems, Conformes: $conformes, No conformes: $noConformes, Porcentaje: $porcentaje%');
  ...
  static pw.Widget _construirItemRelevante(Map<String, dynamic> item) {
    ...
    String descripcionItem = _obtenerDescripcionItemCosecha(item['numero']);
  ...
  static List<Map<String, dynamic>> _extraerItemsRelevantesCosecha(Map<String, dynamic> data) {
    final List<Map<String, dynamic>> relevantes = [];
    
    print('[PDF][COSECHA] Extrayendo items relevantes...');
    print('[PDF][COSECHA] Items a verificar: $ITEMS_COSECHA');
    ...
    for (int i in ITEMS_COSECHA) {
      ...
      print('[PDF][COSECHA] Item $i: respuesta=$respuesta, obs=${observaciones?.isNotEmpty == true ? "SÍ" : "NO"}, foto=${fotoBase64?.isNotEmpty == true ? "SÍ" : "NO"}');
      ...
      if (tieneObservaciones || tieneFoto || esRespuestaNo) {
        ...
        print('[PDF][COSECHA] ✅ Item $i incluido: respuesta=$respuesta, obs=${tieneObservaciones ? "SÍ" : "NO"}, foto=${tieneFoto ? "SÍ" : "NO"}');
      }
    }
    
    print('[PDF][COSECHA] Total items relevantes encontrados: ${relevantes.length}');
    return relevantes;
  }
  ...
  static Uint8List _optimizarImagen(String b64) {
    try {
      ...
      print('[PDF][COSECHA] Error optimizando imagen: $e');
      try {
        final cleaned = b64.replaceAll(RegExp(r"\s+"), '');
        final String b64Only = cleaned.startsWith('data:') ? cleaned.substring(cleaned.indexOf('base64,') + 7) : cleaned;
  ...
  static String _obtenerDescripcionItemCosecha(int numeroItem) {
    // Mapeo de items de cosecha con sus descripciones
    Map<int, String> descripcionesItems = {
      1: 'Preparación del área de cosecha',
      2: 'Identificación de la variedad',
      3: 'Verificación de madurez',
      4: 'Limpieza de herramientas',
      5: 'Organización de contenedores',
      6: 'Preparación del personal',
      7: 'Verificación de condiciones climáticas',
      8: 'Revisión de equipos de protección',
      9: 'Control de calidad inicial',
      10: 'Proceso de cosecha',
      11: 'Selección de frutos',
      12: 'Manejo cuidadoso',
      13: 'Separación por calidad',
      14: 'Pesaje y registro',
      15: 'Embalaje adecuado',
      16: 'Etiquetado correcto',
      17: 'Almacenamiento temporal',
      18: 'Transporte al destino',
      19: 'Documentación del proceso',
      20: 'Limpieza final del área',
    };
    
    return descripcionesItems[numeroItem] ?? 'Item de cosecha $numeroItem';
  }
