...
class AplicacionesPDFService {
  ...
  // Items específicos para aplicaciones (30 items del 1 al 30)
  static final List<int> ITEMS_APLICACIONES = List.generate(30, (index) => index + 1);

  static Future<Uint8List> generate({
    required Map<String, dynamic> data,
    bool obtenerDatosFrescos = false,
  }) async {
    print('[PDF][APLICACIONES] Generando PDF para aplicaciones ID=${data['id']} finca=${data['finca_nombre']}');
    ...
    for (final ruta in rutasPosibles) {
      try {
        ...
        print('[PDF][APLICACIONES] Banner cargado desde: $ruta');
        break;
      } catch (_) {}
    }
    ...
    return pw.Container(
      ...
      child: pw.Row(
        ...
        children: [
          ...
          pw.Text('APLICACIONES', style: pw.TextStyle(color: PdfColors.white, fontWeight: pw.FontWeight.bold, fontSize: 14)),
        ],
      ),
    );
  }
  ...
  static pw.Widget _construirResumenCumplimiento(Map<String, dynamic> data) {
    ...
    print('[PDF][APLICACIONES] Usando porcentaje de la base de datos: $porcentaje%');
    ...
    print('[PDF][APLICACIONES] Total items: $totalItems, Conformes: $conformes, No conformes: $noConformes, Porcentaje: $porcentajeRedondeado%');
  ...
  static Uint8List _optimizarImagen(String b64) {
    try {
      ...
      print('[PDF][APLICACIONES] Error optimizando imagen: $e');
      try {
        final cleaned = b64.replaceAll(RegExp(r"\s+"), '');
        final String b64Only = cleaned.startsWith('data:') ? cleaned.substring(cleaned.indexOf('base64,') + 7) : cleaned;
