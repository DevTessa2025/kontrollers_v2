...
class LaboresTemporalesPDFService {
  ...
  static Future<Uint8List> generate({
    ...
    print('[PDF][LABORES_TEMPORALES] Generando PDF para labores temporales ID=${data['id']} finca=${data['finca_nombre']}');
    ...
    for (final ruta in rutasPosibles) {
      try {
        ...
        print('[PDF][LABORES_TEMPORALES] Banner cargado desde: $ruta');
        break;
      } catch (_) {}
    }

    // Obtener items relevantes para labores temporales
    final List<Map<String, dynamic>> itemsRelevantes = _extraerItemsRelevantesLaboresTemporales(data);
    ...
    print('[PDF][LABORES_TEMPORALES] PDF generado exitosamente');
    return pdf.save();
  }
    ...
    return pw.Container(
      ...
      child: pw.Row(
        ...
        children: [
          ...
          pw.Text('LABORES TEMPORALES', style: pw.TextStyle(color: PdfColors.white, fontWeight: pw.FontWeight.bold, fontSize: 14)),
        ],
      ),
    );
  }
  ...
  static pw.Widget _construirGridImagenes(List<String> images) {
    ...
    for (final img in images) {
      try {
        ...
        print('[PDF][LABORES_TEMPORALES] Error procesando imagen: $e');
      }
    }
  ...
  static List<Map<String, dynamic>> _extraerItemsRelevantesLaboresTemporales(Map<String, dynamic> data) {
    try {
      ...
      print('[PDF][LABORES_TEMPORALES] Error extrayendo items: $e');
      return [];
    }
  }
  ...
  static Uint8List _optimizarImagen(String b64) {
    try {
      ...
      print('[PDF][LABORES_TEMPORALES] Error optimizando imagen: $e');
      try {
        final cleaned = b64.replaceAll(RegExp(r"\s+"), '');
        final String b64Only = cleaned.startsWith('data:') ? cleaned.substring(cleaned.indexOf('base64,') + 7) : cleaned;
