...
import 'checklist_fertirriego_screen.dart';

class ChecklistFertiriegoRecordsScreen extends StatefulWidget {
  @override
  _ChecklistFertiriegoRecordsScreenState createState() =>
      _ChecklistFertiriegoRecordsScreenState();
}

class _ChecklistFertiriegoRecordsScreenState
    extends State<ChecklistFertiriegoRecordsScreen> {
  ...
  List<Map<String, dynamic>> get filteredRecords {
    ...
    return records.where((record) {
      String searchText = _filterText.toLowerCase();
      String finca = (record['finca_nombre'] ?? '').toString().toLowerCase();
      String bloque = (record['bloque_nombre'] ?? '').toString().toLowerCase();
      String usuario =
          (record['usuario_nombre'] ?? '').toString().toLowerCase();

      return finca.contains(searchText) ||
          bloque.contains(searchText) ||
          usuario.contains(searchText);
    }).toList();
  }

  Future<void> _sendToServer(int recordId) async {
    // Verificar que el checklist esté completo antes de enviar
    if (!_isRecordComplete(records.firstWhere((r) => r['id'] == recordId))) {
      bool? continueEdit = await showDialog<bool>(
        context: context,
        builder: (context) => AlertDialog(
          title: Text(
            'Checklist Fertirriego Incompleto',
            style: TextStyle(
                color: Colors.orange[800], fontWeight: FontWeight.bold),
          ),
          content: Text(
              'Este checklist fertirriego no está completo. No se puede enviar al servidor hasta que todos los ítems tengan una respuesta.\n\n¿Desea continuar llenándolo?'),
          actions: [
            TextButton(
    ...
    try {
      ...
      await ChecklistFertiriegoStorageService.markAsEnviado(recordId);

      Fluttertoast.showToast(
        msg: 'Registro fertirriego enviado exitosamente al servidor',
      ...
      Fluttertoast.showToast(
        msg: 'Error enviando registro fertirriego: $e',
        backgroundColor: Colors.red[600],
        textColor: Colors.white,
        toastLength: Toast.LENGTH_LONG,
      );
  ...
  Future<void> _editChecklist(int recordId) async {
    try {
      // CORRECCIÓN: Usar getChecklistAsObjectById en lugar de getChecklistById
      ChecklistFertirriego? checklist =
          await ChecklistFertiriegoStorageService.getChecklistAsObjectById(
              recordId);

      if (checklist == null) {
        Fluttertoast.showToast(
          msg: 'No se pudo cargar el checklist fertirriego para editar',
          backgroundColor: Colors.red[600],
          textColor: Colors.white,
        );
        return;
      ...
      if (record['enviado'] == 1) {
        Fluttertoast.showToast(
          msg:
              'No se puede editar un checklist fertirriego que ya fue enviado al servidor',
      ...
      bool? updated = await Navigator.push<bool>(
        context,
        MaterialPageRoute(
          builder: (context) => ChecklistFertiriegoScreen(
      ...
      Fluttertoast.showToast(
        msg: 'Error cargando checklist fertirriego para editar: $e',
        backgroundColor: Colors.red[600],
        textColor: Colors.white,
      );
    }
  ...
  bool _isRecordComplete(Map<String, dynamic> record) {
    // IDs específicos que existen en fertirriego
    List<int> validIds = [
      1,
      2,
      3,
  ...
  String _generateInsertQuery(Map<String, dynamic> record) {
    ...
    // Agregar todos los items de fertirriego (IDs específicos)
    List<int> validIds = [
      1,
      2,
      3,
    ...
    for (int id in validIds) {
      columnNames.add('item_${id}_respuesta');
      values.add(escapeValue(record['item_${id}_respuesta']));
      columnNames.add('item_${id}_valor_numerico');
      values.add(escapeValue(record['item_${id}_valor_numerico']));
      columnNames.add('item_${id}_observaciones');
      values.add(escapeValue(record['item_${id}_observaciones']));
      columnNames.add('item_${id}_foto_base64');
      values.add(escapeValue(record['item_${id}_foto_base64']));
    }
  ...
  Future<void> _viewChecklistDetails(int recordId) async {
    try {
      ...
      if (checklist == null) {
        Fluttertoast.showToast(
          msg: 'No se pudo cargar el checklist fertirriego',
          backgroundColor: Colors.red[600],
          textColor: Colors.white,
        );
        return;
      ...
      showDialog(
        ...
        builder: (context) => AlertDialog(
          title: Text(
            'Detalles del Checklist Fertirriego',
            style:
          ...
          content: SingleChildScrollView(
            child: Column(
              ...
              children: [
                _buildDetailRow(
                    'Finca:', checklist.finca?.nombre ?? "No seleccionada"),
                _buildDetailRow(
                    'Bloque:', checklist.bloque?.nombre ?? "No seleccionado"),
                _buildDetailRow(
                    'Fecha:',
                    checklist.fecha != null
                ...
                ...checklist.secciones.map((seccion) {
                  int completados = seccion.items
                      .where((item) => item.respuesta != null)
                      .length;
                  int conFotos = seccion.items
                      .where((item) => item.fotoBase64 != null)
                      .length;
                  int conObservaciones = seccion.items
                      .where((item) => item.observaciones != null)
                      .length;
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        Row(
                          ...
                          children: [
                            Text('Items: $completados/${seccion.items.length}',
                                style: TextStyle(fontSize: 11)),
                            Text('Fotos: $conFotos',
                                style: TextStyle(
                                    fontSize: 11, color: Colors.blue[600])),
                            Text('Obs: $conObservaciones',
                                style: TextStyle(
      ...
      Fluttertoast.showToast(
        msg: 'Error cargando detalles fertirriego: $e',
        backgroundColor: Colors.red[600],
        textColor: Colors.white,
      );
    }
  ...
  Widget build(BuildContext context) {
    List<Map<String, dynamic>> displayedRecords = filteredRecords;

    return Scaffold(
      backgroundColor: Colors.white,
      appBar: AppBar(
        title: Text(
          'Registros Fertirriego',
                ...
                Container(
                  ...
                  child: TextField(
                    ...
                    decoration: InputDecoration(
                      hintText: 'Buscar por finca, bloque o usuario...',
                  ...
                  child: Column(
                    children: [
                      Row(
                        ...
                        children: [
                          _buildStatCard(
                              'Total', stats['total'] ?? 0, Colors.blue[600]!),
                          _buildStatCard('Completos', stats['completos'] ?? 0,
                              Colors.green[600]!),
                          _buildStatCard('Incompletos',
                              stats['incompletos'] ?? 0, Colors.orange[600]!),
                          _buildStatCard('Enviados', stats['enviados'] ?? 0,
                              Colors.purple[600]!),
                        ],
                      ),
                    ],
                      ...
                      ? Center(
                          child: Column(
                            ...
                            children: [
                              ...
                              Text(
                                _filterText.isEmpty
                                    ? 'Los checklist de fertirriego aparecerán aquí'
                      ...
                      : ListView.builder(
                          ...
                          itemBuilder: (context, index) {
                            Map<String, dynamic> record =
                                displayedRecords[index];
                            bool enviado = record['enviado'] == 1;
                            bool completo = _isRecordComplete(record);
                            DateTime fecha =
                                DateTime.parse(record['fecha_creacion']);
                                  ...
                                  children: [
                                    ...
                                    Row(
                                      children: [
                                        ...
                                        // Indicador de cumplimiento con icono de fertirriego
                                        Column(
                                          children: [
                                            Container(
                                              width: 60,
                                      ...
                                      children: [
                                        ...
                                        if (!enviado) ...[
                                          Expanded(
                                            child: OutlinedButton.icon(
                                              onPressed: () =>
                                                  _editChecklist(record['id']),
                                              icon: Icon(Icons.edit, size: 16),
                                              label: Text(completo
                                                  ? 'Editar'
                                                  : 'Completar'),
                                              style: OutlinedButton.styleFrom(
                                                foregroundColor: completo
                                                    ? Colors.blue[700]
                                                    : Colors.orange[700],
                                                side: BorderSide(
                                                    color: completo
                                        ...
                                        // Botón de eliminar (comentado por ahora)
                                        // IconButton(
      ...
      floatingActionButton: FloatingActionButton(
        ...
        tooltip: 'Nuevo checklist fertirriego',
      ),
    );
  }
