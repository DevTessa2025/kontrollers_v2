...
import 'checklist_aplicaciones_screen.dart';

class ChecklistAplicacionesRecordsScreen extends StatefulWidget {
  @override
  _ChecklistAplicacionesRecordsScreenState createState() => _ChecklistAplicacionesRecordsScreenState();
}

class _ChecklistAplicacionesRecordsScreenState extends State<ChecklistAplicacionesRecordsScreen> {
  ...
  Future<void> _loadRecords() async {
    ...
    try {
      List<Map<String, dynamic>> loadedRecords = await ChecklistAplicacionesStorageService.getLocalChecklists();
      Map<String, int> loadedStats = await ChecklistAplicacionesStorageService.getLocalStats();
      ...
      Fluttertoast.showToast(
        msg: 'Error cargando registros aplicaciones: $e',
  ...
  Future<void> _sendToServer(int recordId) async {
    ...
    bool isComplete = await ChecklistAplicacionesStorageService.isChecklistComplete(recordId);
    ...
    if (!isComplete) {
      bool? continueEdit = await showDialog<bool>(
        ...
        builder: (context) => AlertDialog(
          title: Text(
            'Checklist Aplicaciones Incompleto',
            style: TextStyle(color: Colors.orange[800], fontWeight: FontWeight.bold),
          ),
          content: Text(
            'Este checklist aplicaciones no está completo. No se puede enviar al servidor hasta que todos los ítems tengan una respuesta.\n\n¿Desea continuar llenándolo?'
          ),
          actions: [
            TextButton(
    ...
    try {
      ...
      await ChecklistAplicacionesStorageService.markAsEnviado(recordId);

      Fluttertoast.showToast(
        msg: 'Registro aplicaciones enviado exitosamente al servidor',
      ...
      Fluttertoast.showToast(
        msg: 'Error enviando registro aplicaciones: $e',
        backgroundColor: Colors.red[600],
        textColor: Colors.white,
        toastLength: Toast.LENGTH_LONG,
      );
  ...
  Future<void> _editChecklist(int recordId) async {
    try {
      ChecklistAplicaciones? checklist = await ChecklistAplicacionesStorageService.getChecklistById(recordId);
      
      if (checklist == null) {
        Fluttertoast.showToast(
          msg: 'No se pudo cargar el checklist aplicaciones para editar',
          backgroundColor: Colors.red[600],
          textColor: Colors.white,
        );
        return;
      ...
      if (record['enviado'] == 1) {
        Fluttertoast.showToast(
          msg: 'No se puede editar un checklist aplicaciones que ya fue enviado al servidor',
          backgroundColor: Colors.orange[600],
          textColor: Colors.white,
        );
        return;
      ...
      bool? updated = await Navigator.push<bool>(
        context,
        MaterialPageRoute(
          builder: (context) => ChecklistAplicacionesScreen(
      ...
      Fluttertoast.showToast(
        msg: 'Error cargando checklist aplicaciones para editar: $e',
        backgroundColor: Colors.red[600],
        textColor: Colors.white,
      );
    }
  ...
  String _generateInsertQuery(Map<String, dynamic> record) {
    ...
    List<String> columnNames = [
      'checklist_uuid',
      'finca_nombre', 'bloque_nombre', 'bomba_nombre',
    ...
    for (int i = 1; i <= 40; i++) { // 40 items para aplicaciones
    ...
    return 'INSERT INTO check_aplicaciones (${columnNames.join(', ')}) VALUES (${values.join(', ')})';
  }
  ...
  Future<void> _deleteRecord(int recordId) async {
    ...
    String confirmMessage = enviado 
        ? '¿Está seguro que desea eliminar este registro aplicaciones enviado? Esta acción no se puede deshacer.'
        : '¿Está seguro que desea eliminar este registro aplicaciones? Esta acción no se puede deshacer.';
    ...
    if (confirm == true) {
      try {
        await ChecklistAplicacionesStorageService.deleteLocalChecklist(recordId);
        await _loadRecords();

        Fluttertoast.showToast(
          msg: 'Registro aplicaciones eliminado correctamente',
          backgroundColor: Colors.orange[600],
          textColor: Colors.white,
        );
      } catch (e) {
        Fluttertoast.showToast(
          msg: 'Error eliminando registro aplicaciones: $e',
          backgroundColor: Colors.red[600],
          textColor: Colors.white,
        );
      }
  ...
  Future<void> _viewChecklistDetails(int recordId) async {
    try {
      ...
      if (checklist == null) {
        Fluttertoast.showToast(
          msg: 'No se pudo cargar el checklist aplicaciones',
          backgroundColor: Colors.red[600],
          textColor: Colors.white,
        );
        return;
      ...
      showDialog(
        ...
        builder: (context) => AlertDialog(
          title: Text(
            'Detalles del Checklist Aplicaciones',
            style: TextStyle(color: Colors.orange[800], fontWeight: FontWeight.bold),
          ),
          content: SingleChildScrollView(
            child: Column(
              ...
              children: [
                ...
                _buildDetailRow('Bomba:', checklist.bomba?.nombre ?? "No seleccionada"),
      ...
      Fluttertoast.showToast(
        msg: 'Error cargando detalles aplicaciones: $e',
        backgroundColor: Colors.red[600],
        textColor: Colors.white,
      );
    }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: _isLoading
          ? Center(
              child: Column(
                ...
                children: [
                  ...
                  SizedBox(height: 16),
                  Text(
                    'Cargando registros aplicaciones...',
          ...
          : Column(
              children: [
                // Estadísticas mejoradas con tema naranja
                Container(
                      ...
                      ? Center(
                          child: Column(
                            ...
                            children: [
                              ...
                              SizedBox(height: 16),
                              Text(
                                'No hay registros aplicaciones guardados',
                                style: TextStyle(
                                  fontSize: 18,
                              ...
                              SizedBox(height: 8),
                              Text(
                                'Los checklist aplicaciones guardados aparecerán aquí',
                                style: TextStyle(
                                  fontSize: 14,
